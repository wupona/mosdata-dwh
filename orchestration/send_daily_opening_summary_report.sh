...
# ==============================================================================
# Script: send_daily_opening_summary_report.sh
# But: Envoyer le dernier sommaire stock "opening_grouped_by_location_*.csv" par email
#      en forçant un expéditeur autorisé (From + envelope-from).
# ==============================================================================

BASE_DIR="/mnt/c/Blissydah"
REPORT_DIR="${BASE_DIR}/output/validate_opening"
RECIPIENTS_FILE="${BASE_DIR}/config/report_recipients.txt"
LOG_FILE="${BASE_DIR}/logs/send_mail_opening_summary.log"

# IMPORTANT: doit être une adresse "owned by user" sur le SMTP blissydah
FROM_ADDR="norbert.wupona@blissydah.com"

SUBJECT_PREFIX="[Blissydah] Sommaire stock (Opening) - Grouped by location"
...
# Trouver le dernier fichier opening_grouped_by_location_*.csv
LATEST_FILE="$(ls -1t "${REPORT_DIR}"/opening_grouped_by_location_*.csv 2>/dev/null | head -n 1 || true)"
if [[ -z "${LATEST_FILE}" ]]; then
  log "ERREUR: Aucun fichier opening_grouped_by_location_*.csv trouvé dans ${REPORT_DIR}"
  exit 1
fi
...
Veuillez trouver ci-joint le sommaire du stock (Opening) regroupé par emplacement.
...
Remarque:
Ce rapport est un sommaire (Qty, Cost, Price) par emplacement et sert de contrôle rapide.
